...
Currently supports:
  ...
  - manage_labeled_gh_event: Orchestrate all label-sync steps in a single invocation.
...
Usage:
  ...
  python3 scripts/jira_sync_logic.py --action manage_labeled_gh_event
...
Environment variables (for manage_labeled_gh_event):
  PR_TITLE           - The pull request title
  PR_BODY            - The pull request body
  PR_NUMBER          - The PR number
  TRIGGERING_LABEL   - The label that triggered this event
  OWNER_REPO         - The GitHub repository (owner/repo)
  GITHUB_TOKEN       - GitHub token for PR API calls
  JIRA_AUTH          - Jira auth credential "<user email>:<api_token>"
